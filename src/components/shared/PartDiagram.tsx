...
function AirBaffleDiagram({ surfaceArea, params }: PartDiagramProps["params"] & { surfaceArea: number }) {
  const l = (params.length / 10).toFixed(1);
  const w = (params.width / 10).toFixed(1);
  return (
    <svg viewBox="0 0 600 220" className="w-full h-full">
      ...
      {/* Info */}
      <text x="20" y="155" fill="#64748B" fontSize="9">
        Surface Area: ~{surfaceArea.toFixed(1)} cm²
      </text>
      <text x="20" y="170" fill="#64748B" fontSize="9">
        Thickness: {params.thickness} mm
      </text>
      <text x="20" y="185" fill="#64748B" fontSize="9">
        Vanes: {params.extraDim1}
      </text>
...
function TopCoverDiagram({ surfaceArea, params }: PartDiagramProps["params"] & { surfaceArea: number }) {
  const l = (params.length / 10).toFixed(1);
  const w = (params.width / 10).toFixed(1);
  return (
    <svg viewBox="0 0 600 220" className="w-full h-full">
      ...
      <text x="20" y="200" fill="#64748B" fontSize="9">
        SA: ~{surfaceArea.toFixed(1)} cm² · Cutout: {params.extraDim1}%
      </text>
...
function RiserCageDiagram({ surfaceArea, params }: PartDiagramProps["params"] & { surfaceArea: number }) {
  const l = (params.length / 10).toFixed(1);
  const w = (params.width / 10).toFixed(1);
  return (
    <svg viewBox="0 0 600 220" className="w-full h-full">
      <rect x="0" y="0" width="600" height="220" fill="#0F172A" rx="8" />
      {/* 3D isometric-style riser cage */}
      ...
      <text x="20" y="40" fill="#64748B" fontSize="9">
        SA: ~{surfaceArea.toFixed(1)} cm² · Slots: {params.slotCount ?? 2}
      </text>
      <text x="20" y="55" fill="#64748B" fontSize="9">
        Depth: {params.extraDim1} mm · Holes: {params.extraDim2 ?? 0}
      </text>
      {/* Slot labels */}
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-border shadow-sm">
      <div className="h-[200px]">
        <Diagram surfaceArea={surfaceArea} params={params} />
      </div>
